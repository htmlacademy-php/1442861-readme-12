...
function cutContent ($long_content, $max_length =300) {
    $current_length = mb_strlen($long_content);

    if ($current_length>$max_length) {
         
        $long_content = explode (' ', $long_content);
        while ($current_length >$max_length) {
            $word = array_pop($long_content);
            $current_length-=(mb_strlen($word)+1);
        }
        $short_content = implode(' ',$long_content).'...';    
    } 
    else {$short_content = $long_content;}
